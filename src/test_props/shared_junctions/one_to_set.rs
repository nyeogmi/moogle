...
use crate::test_props::properties::{pair_unique, bwd_unique}; 
...
impl crate::OneToSet<u16, i16> {
    ...
    fn prime<'a>(&'a self, phase: &Phase, iter_bank: &mut IterBank<'a>) {
        let items: Vec<u16> = match phase {
            Phase::Insert{fwd, ..} => 
                items_only(&fwd).iter().take(DESIRED_N_ITERATORS).map(|(a, _)| *a).collect(),
            Phase::Remove{fwd, ..} => 
                items_only(&fwd).iter().take(DESIRED_N_ITERATORS).map(|(a, _)| *a).collect(),
            Phase::Expunge{fwd, ..} =>
                items_only(&fwd).into_iter().take(DESIRED_N_ITERATORS).collect(),
        };
        for i in items { iter_bank.add_iterator(self.fwd().get(i).iter()) }
    }
